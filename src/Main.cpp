...
json decode_bencoded_dict(const std::string& encoded_value, uint& index) {
  ...
  while (encoded_value[index] != 'e') {
    ...
    json second;
    if (encoded_value[index] == 'i') {
      second = decode_bencoded_number(encoded_value, index);
      dict[first] = second;
      done = false;
    } else if (encoded_value[index] == 'l') {
      second = decode_bencoded_list(encoded_value, index);
      dict[first] = second;
      done = false;
    } else {
      if (done) {
        second = decode_bencoded_string(encoded_value, index);
        dict[first] = second;
        done = false;
      } else {
        first = decode_bencoded_string(encoded_value, index);
        done = true;
      }
    }
  }
